...
for (s in species ) {
  if (s=="POSE"){
    raw.readbiomass<-read.csv(paste0("Data/",s,"_Data/SA_",s,"_Biomass_08_18_25.csv"))
  }
  # if (s=="POSE"){
  #   raw.readbiomass<-read.csv("Data/",s,"_Data/SA_","_Biomass_08_18_25.csv")
  # }
  if (s=="ACMI"){
    raw.readbiomass<-read.csv(paste0("Data/",s,"_Data/SA_",s,"_Biomass_05_08_26.csv"))
  }
  #if (s=="ELEL"){
   # raw.readbiomass<-read.csv("Data/",s,"_Data/SA_","_Biomass_08_18_25.csv")
  #}
  data.list[[s]]<-raw.readbiomass
}
...
for(s in species){
  raw.readbiomass <- data.list[[s]]
  raw.readbiomass2<- raw.readbiomass[-c(69:84),]
  clean.data.list[[s]]<- raw.readbiomass2
}
...
for(s in species){
  ...
  raw.readbiomass2 <- clean.data.list[[s]] 
  ...
  avg.mass.list[[s]]<- raw.readbiomass2
}
...
shootweightfit.full.list <- list()
shootweight.output <- list()
...
for (s in species){
  print(s)
  temp_agg_sh <- avg.mass.list[[s]]
  shootmass_cols<- c("Treatment","Day_Scale_7","Night_Scale_7","mass_SH_diff")
  shootmass<- temp_agg_sh[,shootmass_cols]
  
  shootWeightfull<-glm(mass_SH_diff~Treatment+Day_Scale_7+Night_Scale_7,
                    family=gaussian(link = "identity"),data=shootmass)
  print(summary(shootWeightfull))
  ...
  shootweightfit.full.list[[s]] <- shootWeightfull

  shootweightrast <- data.frame(summary(shootWeightfull)$coefficients)
  shootweightrast$Species <- s #change to s inside loop
  shootweightrast$coefficients <- rownames(shootweightrast)
  shootweight.output[[s]]<- shootweightrast
...
names(output.all.shootweight)

str(output.all.shootweight$coefficients)
unique(output.all.shootweight$coefficients)

output.all.shootweight$coefficients<-factor(output.all.shootweight$coefficients,
...
output.all.shootweight$sigvar <-
  ifelse(output.all.shootweight$Pr...t.. < 0.05, 1, 0)

shootweight_sig<-output.all.shootweight[which(output.all.shootweight$sigvar==1),]         

ShootWeight_Rasters <- ggplot(output.all.shootweight[-which(output.all.shootweight$coefficients=="(Intercept)"),],
       aes(x=Species,y=coefficients))+
  geom_raster(aes(fill=Estimate))+
  geom_tile(data =shootweight_sig[-which(shootweight_sig$coefficients=="(Intercept)"),], 
            aes(x=Species,y=coefficients, fill = sigvar),fill="transparent",
            col="black",linewidth=2)+
  scale_fill_gradient2(low = "red", high = "blue", mid = "white",
                       midpoint = 0, space = "Lab",
                       name="Effect")+
  geom_text(aes(label = paste0(round(Estimate,7))))+
  ylab("")+
  xlab("")+
...
rootweightfit.full.list <- list()
rootweight.output <- list()
...
for (s in species){
  print(s)
  temp_agg_rt <- avg.mass.list[[s]]
  rootmass_cols <- c("Treatment", "Day_Scale_7", "Night_Scale_7", "mass_RT_diff")
  rootmass <- temp_agg_rt[,rootmass_cols]
  
  rootWeightfull <- glm(mass_RT_diff~Treatment+Day_Scale_7+Night_Scale_7,
                        family=gaussian(link = "identity"),data=rootmass)
  print(summary(rootWeightfull))
  ...
  rootweightfit.full.list[[s]] <- rootWeightfull
  
  rootweightrast <- data.frame(summary(rootWeightfull)$coefficients)
  rootweightrast$Species <- s #change to s inside loop
  rootweightrast$coefficients <- rownames(rootweightrast)
  rootweight.output[[s]]<- rootweightrast
...
#Save for later when we have all
output.all.rootweight <- do.call(rbind, rootweight.output)

names(output.all.rootweight)

str(output.all.rootweight$coefficients)
unique(output.all.rootweight$coefficients)

output.all.rootweight$coefficients<-factor(output.all.rootweight$coefficients,
                                     levels = c("TreatmentH:Day_Scale_7", 
                                                "TreatmentL:Day_Scale_7",
                                                "TreatmentW:Day_Scale_7",
                                                "Day_Scale_7:Night_Scale_7",
                                                "(Intercept)", "TreatmentW",
                                                "TreatmentL","TreatmentH",
                                                "Day_Scale_7", "Night_Scale_7"),
                                     labels = c("High SA Treatment \n Day Temperature",
                                                "Low SA Treatment \n Day Temperature",
                                                "Water Treatment \n Day Temperature",
                                                "Day Temperature \n Night Temperature",
                                                "(Intercept)", "Water Treatment", 
...
output.all.rootweight$sigvar <-
  ifelse(output.all.rootweight$Pr...t.. < 0.05, 1, 0)

rootweight_sig<-output.all.rootweight[which(output.all.rootweight$sigvar==1),]         

RootWeight_Rasters <- ggplot(output.all.rootweight[-which(output.all.rootweight$coefficients=="(Intercept)"),],
       aes(x=Species,y=coefficients))+
  geom_raster(aes(fill=Estimate))+
  geom_tile(data =rootweight_sig[-which(rootweight_sig$coefficients=="(Intercept)"),], 
            aes(x=Species,y=coefficients, fill = sigvar),fill="transparent",
            col="black",linewidth=2)+
  scale_fill_gradient2(low = "red", high = "blue", mid = "white",
                       midpoint = 0, space = "Lab",
...
Bmass.shoot.plots <- list()

for (s in species){
  
  Shootmass.plot<- avg.mass.list[[s]]
  
  night.t.S.mass.boxplots <- ggplot(Shootmass.plot, aes(factor(round(Night_Scale_7,1)), log(mass_SH_diff), fill=Treatment)) +
    geom_smooth(se=F,aes(group=Treatment,col=Treatment), span= 1.5,linewidth=0.5)+
    geom_boxplot() +
    ...
    labs(title = paste0(s),
         x="Night Temperature",
         y="Shoot Mass Difference")+
    theme_minimal(base_size=10)+
    scale_color_manual(values=c("#CCCCCC", "#33a02c","#b2df8a","#1f78b4"))+    
    scale_fill_manual(values=c("#CCCCCC", "#33a02c","#b2df8a","#1f78b4"))
  
  Bmass.shoot.plots[[s]] <- night.t.S.mass.boxplots
}

print(Bmass.shoot.plots[["POSE"]])

# IDK WHAT WAS HAPPENING DOWN HERE
# shootmass2<- shootmass[-c(1:4,65:68),] #do we remember why I did th
# 
