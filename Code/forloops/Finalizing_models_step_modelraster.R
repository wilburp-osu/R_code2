...
for (s in species){
  ...
  test<-data.frame(summary(rootDaysfull)$coefficients)
  test$Species<-s
  test$coefficients<-rownames(test)
  rootDays.output[[s]]<-test
  
  
  
}

rootDays.output.all<-do.call(rbind,rootDays.output)

ggplot(rootDays.output.all[-which(rootDays.output.all$coefficients=="(Intercept)"),],aes(x=Species,y=coefficients))+
...
for (s in species){
  ...
  emergplots <- ggplot(pnew.data.shoot,aes(x=Day_Scale_7,y=Night_Scale_7))+
    geom_raster(aes(fill=Prediction_back))+
    facet_grid(.~Treatment)+
    scale_fill_gradient2(low = "blue", high = "red", mid = "white",
                         midpoint = mid.pointss, space = "Lab",
