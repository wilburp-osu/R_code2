...
# models #####
## root rate glmmTMB ####
#rootratefit<-glmmTMB(Avg_Root_rate~(1|Group)+Day_Scale_7+Night_Scale_7+Day_Scale_7:Treatment+Night_Scale_7:Treatment+Treatment,
...
for (s in species) {
  temp_agg<-clean.data.list[[s]]
  temp_agg2<-temp_agg[-which(temp_agg$Avg_Root_rate==0),]
  rootratefit.full<-glmmTMB(I(Avg_Root_rate)~(1|Group)+Day_Scale_7+Night_Scale_7+
                              Day_Scale_7:Treatment+Night_Scale_7:Treatment+Treatment+
  ...
  #rootratefit<-glmmTMB(I(Avg_Root_rate+0.0001)~(1|Group)+Day_Scale_7+Night_Scale_7+
  #                     Day_Scale_7:Treatment+Night_Scale_7:Treatment+Treatment+
  #                     Day_Scale_7:Night_Scale_7,
...
for (s in species){
  ...
  print(ggplot(pnew.data,aes(x=Avg_Root_rate,y=Prediction_back,col=Treatment))+
          geom_point()+geom_abline(slope=1)+theme_minimal()+ labs(title = paste0(s)))
  plotly_day<-matrix(pnew.data$Day_Scale_7[which(pnew.data$Treatment=="H")],byrow=T,ncol=7)
  plotly_night<-matrix(pnew.data$Night_Scale_7[which(pnew.data$Treatment=="H")],byrow=T,ncol=7)
  ...
  fig<-plot_ly(showscale=F,type="surface", x=~plotly_day,y=~plotly_night) %>% 
    layout(title=paste0(s,"_Root_growth_rate"))
...
for (s in species) {
  temp_agg<-clean.data.list[[s]]
  temp_agg2<-temp_agg[-which(temp_agg$Avg_Shoot_rate==0),]
  shootratefit.full<-glmmTMB(I(Avg_Shoot_rate)~(1|Group)+Day_Scale_7+Night_Scale_7+
                               Day_Scale_7:Treatment+Night_Scale_7:Treatment+Treatment+
                               Day_Scale_7:Night_Scale_7+
                               Day_Scale_7:Night_Scale_7:Treatment,
                             family=Gamma(link="log"),data=temp_agg2,
  ...
  # shootratefit<-glmmTMB(I(Avg_Shoot_rate+0.0001)~(1|Group)+Day_Scale_7+Night_Scale_7+
  #                         Day_Scale_7:Treatment+Night_Scale_7:Treatment+Treatment+
  #                         Day_Scale_7:Night_Scale_7,
  #                       family=Gamma(link="log"),data=temp_agg)
  # 
...
### 3D figs #####
for (s in species){
  #shootratefit <- shoot.rate.ful.list[[s]]
  shootratefit.full <- shoot.rate.full.list[[s]]
  temp_agg<-agg.list[[s]]
  pnew.data.sh <- agg.list[[s]]
  
  pnew.data.sh$Prediction<-predict(shootratefit.full,pnew.data.sh,allow.new.levels=TRUE)
  ...
  fig<-plot_ly(showscale=F,type="surface", x=~plotly_day,y=~plotly_night) %>% 
    layout(title=paste0(s,"_Shoot_growth_rate"))
  fig <- fig %>% add_surface(z = ~plotly_zC, opacity = 0.5, colorscale = list(c(0,1),c("rgb(107,255,184)","rgb(0,124,90)"))) #green
  fig <- fig %>% add_surface(z = ~plotly_zW, opacity = 0.5, colorscale = list(c(0,1),c("rgb(129,212,247)","rgb(12,177,247)"))) #blue
  fig <- fig %>% add_surface(z = ~plotly_zL, opacity = 0.5, colorscale = list(c(0,1),c("rgb(247,226,157)","rgb(255,192,7)"))) #yellow
  fig <- fig %>% add_surface(z = ~plotly_zH, opacity = 0.5, colorscale = list(c(0,1),c("rgb(255,107,184)","rgb(128,0,64)"))) #pink
