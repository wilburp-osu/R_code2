...
for (s in species){
  ...
  #unless...
  #bind.temp <- cbind(days_til_emerg, 
   #                     days_til_germ,
    #                    germ_rate,
     #                   emerg_rate)
      #                  ~unique_ID + 
  ...
  #indiv <- unique(bind.temp)  
  
  #if (any(shorttemp$days_til_emerg = NA)) NA
  #do I try if () to refill the NAs
  #then same for 0s in
...
###this is close, just need to average all of the seeds with zeros
# how do I do this? we will find out

view(clean.data.list.short)

cleandataframe <- as.data.frame(clean.data.list.short)
view(cleandataframe)
...
aglistframe <- as.data.frame(agg.list2)
view(aglistframe)
...
##when do I add (1|Group) back In?
rootDaysfit.list <-list()
rootDaysfit.full.list <- list()
for (s in species){
  print(s)
  temp_agg2 <- agg.list[[s]]
  ...
  rootDaysfull<-glmmTMB(days_til_germ~Day_Scale_7+Night_Scale_7+
...
for (s in species){
  ...
  shootDaysfull<-glmmTMB(days_til_emerg~Day_Scale_7+Night_Scale_7+
                          Day_Scale_7:Treatment+Night_Scale_7:Treatment+
                          Treatment+Day_Scale_7:Night_Scale_7+
                          Day_Scale_7:Night_Scale_7:Treatment,
                        family=nbinom2(link = "log"),ziformula=~1,data=temp_agg2,
